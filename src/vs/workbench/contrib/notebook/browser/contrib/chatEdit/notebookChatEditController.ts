...
class NotebookChatEditorController extends Disposable {
	...
	constructor(
		...
		this._register(autorun(r => {
			// const inserted = insertedCells.read(r);
			const entry = entryObs.read(r);
			const inserted = entry?.diffInfo.read(r).filter(d => d.type === 'insert');
			const model = notebookModel.read(r);
			if (!entry || !model || !inserted) {
				return;
			}
			if (!inserted.length || entry.state.read(r) !== WorkingSetEntryState.Modified) {
				this.deletedCellDecorator.clear();
				return;
			}
		...
		this._register(autorun(r => {
			...
			const hasDeletedDiffs = diffInfo.some(d => d.type === 'delete');
			if (!hasDeletedDiffs || entry.state.read(r) !== WorkingSetEntryState.Modified) {
				this.deletedCellDecorator.clear();
				return;
			}
